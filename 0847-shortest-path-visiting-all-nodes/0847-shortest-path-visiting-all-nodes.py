class Solution:
    def shortestPathLength(self, graph: List[List[int]]) -> int:
        ...
        # NOTE this does not prevent revisiting the same node again, it just prevents
        # revisiting it with the same STATE!
        
        # Since the input size is restricted, we can use a number to encode
        # which does have been visited -- the i-th bit is on iff node i has been visited
        
        # conceptually masks[k] indicates that only node k has been visited
        masks = [1 << i for i in range(nodeCount)]
        # used to check whether all nodes have been visited (111...111)
        ...
        while queue:
            ...
            for _ in range(len(queue)):
                ...
                for nb in graph[currentNode]:
                    ...
                    # pre-check here for efficiency, as each steps increment may results
